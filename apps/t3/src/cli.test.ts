...
describe("parseCliOptions", () => {
  ...
  it("trims tabbed environment variable port values before parsing", () => {
    const options = parseCliOptions(
      [],
      {
        T3_BACKEND_PORT: "\t5001\t",
  ...
  it("treats falsey T3_NO_OPEN values as disabled", () => {
    const options = parseCliOptions(
      [],
      {
        T3_NO_OPEN: "off",
  ...
  it("treats trimmed uppercase falsey values for T3_NO_OPEN as disabled", () => {
    const options = parseCliOptions(
      [],
      {
        T3_NO_OPEN: "\tOFF\t",
...
describe("parseByteRangeHeader", () => {
  ...
  it("tolerates tab whitespace around separators", () => {
    expect(parseByteRangeHeader("bytes\t=\t0\t-\t9", 100)).toEqual({
      start: 0,
      end: 9,
    });
