...
export default function ChatView() {
  ...
  const persistScriptsUpdate = useCallback(
    async (
      ...
      try {
        const updated = await nativeApi.projects.updateScripts({
          id: project.id,
          scripts: nextScripts,
        });
        ...
        if (input.keybinding) {
          const keybindingUpdate = await nativeApi.server.upsertKeybinding({
            key: input.keybinding,
            command: commandForProjectScript(scriptCommandId),
          });
  ...
  const saveProjectScript = useCallback(
    async (input: NewProjectScriptInput) => {
      ...
      await persistScriptsUpdate(activeProject, api, nextScripts, nextId, input);
    },
    [activeProject, api, dispatch, persistScriptsUpdate],
  );
  const updateProjectScript = useCallback(
    async (scriptId: string, input: NewProjectScriptInput) => {
      ...
      if (!isElectron || !api) return;
      await persistScriptsUpdate(activeProject, api, nextScripts, scriptId, input);
    },
    [activeProject, api, dispatch, persistScriptsUpdate],
  );
